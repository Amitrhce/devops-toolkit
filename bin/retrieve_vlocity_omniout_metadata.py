...
def retrieve_vlocity_metadata(vlocity_yaml_file_path, remotes, environment, row, retrieved_components, output_folder):
   ...
   retrieve_cmd = 'vlocity packExportSingle -job="' + vlocity_yaml_file_path + '" -sf.username="'  + remote['username'] + '" -sf.password="' + remote['password'] + '" -sf.loginUrl="' + remote['serverUrl'] + '"'
...
def main():
   ...
   for row in csv_reader:
      # OmniScript filter
      if(row['Vlocity_Type__c'] == 'OmniScript'):
         counter = counter + 1
