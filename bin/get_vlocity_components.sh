...
if [ ! -z "$items" ]; then
   ...
   query="select Environment__c, Backlog_Item__r.Id, Backlog_Item__r.Name, Backlog_Item__r.Vlocity_Deployment_Status__c, Name, Vlocity_Type__c, OmniScript_Type__c, OmniScript_SubType__c, Version__c, LastModifiedBy.Name from Vlocity_Component__c where Backlog_Item__r.Name in($componentList)"
