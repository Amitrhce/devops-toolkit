...
if [ ! -z "$outputFolder"  ];then
  ...
  if [ "$dev01_components_count" -gt 0 ] && [ "$dev02_components_count" -gt 0 ];then
    ...
    if [ ! -z "$target" ];then
      ...
      for((j=0; j<${#target_array[@]}; ++j)); do  
        log "validating ${itemList} dev01 components to ${target_array[j]}"
        force-dev-tool validate -d "${outputFolder}_dev01/src" "${target_array[j]}"

        log "validating ${itemList} dev02 components to ${target_array[j]}"
        force-dev-tool validate -d "${outputFolder}_dev02/src" "${target_array[j]}"
      done
    else
      log "No target environment specified, nothing will be validateed, if you want to validate the package later on, please run"
      log "force-dev-tool validate -d ${outputFolder}_dev01/src [TARGET]"
      log "force-dev-tool validate -d ${outputFolder}_dev02/src [TARGET]"
    fi
  else
    ...
    if [ "$dev01_components_count" -gt 0 ];then
      ...
      if [ ! -z "$target" ];then
        for((j=0; j<${#target_array[@]}; ++j)); do  
          log "validating ${itemList} dev01 components to ${target_array[j]}"
          force-dev-tool validate -d "${outputFolder}/src" "${target_array[j]}"
        done
      else
        log "No target environment specified, nothing will be validateed, if you want to validate the package later on, please run"
        log "force-dev-tool validate -d ${outputFolder}/src [TARGET]"
      fi
    else
  ...
  for((i=0; i<${#items[@]}; ++i)); do
    ...
    if [ "$dev01_components_count" -gt 0 ] && [ "$dev02_components_count" -gt 0 ];then
      ...
      if [ ! -z "$target" ];then
        for((j=0; j<${#target_array[@]}; ++j)); do 
          log "validating ${items[i]} dev01 components to ${target_array[j]}"
          force-dev-tool validate -d "packages/${items[i]}_dev01/src" "${target_array[j]}"

          log "validating ${items[i]} dev02 components to ${target_array[j]}"
          force-dev-tool validate -d "packages/${items[i]}_dev02/src" "${target_array[j]}"
        done
      else
        log "No target environment specified, nothing will be validateed, if you want to validate the package later on, please run"
        log "force-dev-tool validate -d packages/${items[i]}_dev01/src [TARGET]"
        log "force-dev-tool validate -d packages/${items[i]}_dev02/src [TARGET]"
      fi
    else
      ...
      if [ "$dev01_components_count" -gt 0 ];then
        ...
        if [ ! -z "$target" ];then
          for((j=0; j<${#target_array[@]}; ++j)); do
            log "validating ${items[i]} dev01 components to ${target_array[j]}"
            force-dev-tool validate -d "packages/${items[i]}/src" "${target_array[j]}"
          done
        else
          log "No target environment specified, nothing will be validateed, if you want to validate the package later on, please run"
          log "force-dev-tool validate -d packages/${items[i]}/src [TARGET]"
        fi
      else
