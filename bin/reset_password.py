...
def get_users(environment, query_filter):
   ...
   users_csv = subprocess.check_output('force-dev-tool bulk export "select Id, Name  from User where ' + query_filter + '" ' + environment, shell=True)
...
def main():
   parser = argparse.ArgumentParser(description='Reset user\'s password.\n' +
                                                'Example:\n' + 
                                                '\treset_password.py -s dev01 -q "Lastname = \'Test\' and Username like \'%three.dev01\'"',
   ...
   if(len(users) > 0):
      for user in users:
         apex = "System.resetPassword('" + user['Id'] + "', true);"
         print "Running command: " + apex
         print runAnonymousApex(apex, args.source_env)
   else:
