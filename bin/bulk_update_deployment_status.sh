...
print_help(){
    ...
    echo "	-d	debug mode"
    echo "	-i	input file"
    echo "	-s	status string"
    echo 
    echo "Example"
    echo "	$(basename $0) -s "deva;devb;cita;citc" Item-01234 Item-43210"
    echo "	$(basename $0) -h"
...
if [ ! -z "$items" ]; then
   ...
   query="DevOpsUtils.updateBacklogItemDeploymentStatuses(new List<String>{$componentList},'$status_string');"
